...
export const NewsTextCard = ({ news, userId, displayOffcanvas }) => {
  // useToken
  ...
  return (
    <Link to={news.link} className="noFontStyle">
      <div className="newsTextCard">
        <div className="newsTextCardTitle">{news.tittle}</div>
        <div className="newsTextCardDetails">
          <Button onClick={handleClick} className="newsTextCardLikeBtn" variant="link">
            {liked ? (
              <ThumbUpFilledIcon color={'var(--bs-primary)'} width={12} height={12} />
            ) : (
              <ThumbUpIcon
          ...
          <div className="newsTextCardDate">{formatDate(new Date(news.writingTime))}</div>
        </div>
      </div>
    </Link>
  );
